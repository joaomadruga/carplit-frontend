...
export default function DistanceInput({ TextTitle, TextSubtitle, pathInfoState, InputPlaceHolder, ...props }) {
    return (
        <InputWithTitleSubtitleStyle {...props}>
            <Title>{TextTitle}</Title>
            <Subtitle>{TextSubtitle}</Subtitle>
            <Input InputPlaceHolder={InputPlaceHolder} pathInfoState={pathInfoState} SufixValue={"km"} InputFormatter={Constants.ConsumeInputFormatter}/>
