...
export default function AddCarpool({ navigation, route }) {
    ...
    const [isSoloCarpool, setIsSoloCarpool] = useState(false);
    ...
    return (
        <ScrollView style={{backgroundColor: Constants.colors.gray[0]}}>
            <PaddingContent>
                ...
                <SwitchButton isSoloCarpool={isSoloCarpool} isLeftSelectedState={{isLeftSelected: isLeftSelected, setIsLeftSelected: setIsLeftSelected}}/>
                <SwitchPage props={{ carpoolPrice, listOfCarpools, setListOfCarpools, tempListOfRiders: listOfRiders, isLeftSelected, loginInfo, title, totalDistance, navigation, pathId: _id, isSoloCarpool, setIsSoloCarpool }}/>
            </PaddingContent>
        </ScrollView>
    );
}